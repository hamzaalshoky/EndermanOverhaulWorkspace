...
@OnlyIn(Dist.CLIENT)
public class OceanEndermanEyesLayer<E extends OceanEndermanEntity> extends GeoLayerRenderer<E> {

    public OceanEndermanEyesLayer(IGeoRenderer<E> entityRendererIn) {
    ...
    public void render(PoseStack stack, MultiBufferSource buffer, int packedLightIn, E creeper, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
        GeoModel normalModel = this.getEntityModel().getModel(OceanEndermanModel.MODEL);
        VertexConsumer glowConsumer = buffer.getBuffer(RenderTypes.getTransparentEyes(OceanEndermanModel.GLOW_TEXTURE));
