...
@OnlyIn(Dist.CLIENT)
public class SavannaEndermanEyesLayer<E extends SavannaEndermanEntity> extends GeoLayerRenderer<E> {

    public SavannaEndermanEyesLayer(IGeoRenderer<E> entityRendererIn) {
    ...
    public void render(PoseStack stack, MultiBufferSource buffer, int packedLightIn, E creeper, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
        GeoModel normalModel = this.getEntityModel().getModel(SavannaEndermanModel.MODEL);
        VertexConsumer glowConsumer = buffer.getBuffer(RenderTypes.getTransparentEyes(SavannaEndermanModel.GLOW_TEXTURE));
