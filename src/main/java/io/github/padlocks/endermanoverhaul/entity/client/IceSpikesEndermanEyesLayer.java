...
@OnlyIn(Dist.CLIENT)
public class IceSpikesEndermanEyesLayer<E extends IceSpikesEndermanEntity> extends GeoLayerRenderer<E> {

    public IceSpikesEndermanEyesLayer(IGeoRenderer<E> entityRendererIn) {
    ...
    public void render(PoseStack stack, MultiBufferSource buffer, int packedLightIn, E creeper, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
        GeoModel normalModel = this.getEntityModel().getModel(IceSpikesEndermanModel.MODEL);
        VertexConsumer glowConsumer = buffer.getBuffer(RenderTypes.getTransparentEyes(IceSpikesEndermanModel.GLOW_TEXTURE));
