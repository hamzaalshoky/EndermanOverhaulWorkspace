...
@OnlyIn(Dist.CLIENT)
public class SoulsandValleyEndermanEyesLayer<E extends SoulsandValleyEndermanEntity> extends GeoLayerRenderer<E> {

    public SoulsandValleyEndermanEyesLayer(IGeoRenderer<E> entityRendererIn) {
    ...
    public void render(PoseStack stack, MultiBufferSource buffer, int packedLightIn, E creeper, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
        GeoModel normalModel = this.getEntityModel().getModel(SoulsandValleyEndermanModel.MODEL);
        VertexConsumer glowConsumer = buffer.getBuffer(RenderTypes.getTransparentEyes(SoulsandValleyEndermanModel.GLOW_TEXTURE));
