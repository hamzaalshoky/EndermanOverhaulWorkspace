...
@OnlyIn(Dist.CLIENT)
public class DesertEndermanEyesLayer<E extends DesertEndermanEntity> extends GeoLayerRenderer<E> {

    public DesertEndermanEyesLayer(IGeoRenderer<E> entityRendererIn) {
    ...
    public void render(PoseStack stack, MultiBufferSource buffer, int packedLightIn, E creeper, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
        GeoModel normalModel = this.getEntityModel().getModel(DesertEndermanModel.MODEL);
        VertexConsumer glowConsumer = buffer.getBuffer(RenderTypes.getTransparentEyes(DesertEndermanModel.GLOW_TEXTURE));
