...
@OnlyIn(Dist.CLIENT)
public class FlowerFieldsEndermanEyesLayer<E extends FlowerFieldsEndermanEntity> extends GeoLayerRenderer<E> {

    public FlowerFieldsEndermanEyesLayer(IGeoRenderer<E> entityRendererIn) {
    ...
    public void render(PoseStack stack, MultiBufferSource buffer, int packedLightIn, E creeper, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
        GeoModel normalModel = this.getEntityModel().getModel(FlowerFieldsEndermanModel.MODEL);
        VertexConsumer glowConsumer = buffer.getBuffer(RenderTypes.getTransparentEyes(FlowerFieldsEndermanModel.GLOW_TEXTURE));
