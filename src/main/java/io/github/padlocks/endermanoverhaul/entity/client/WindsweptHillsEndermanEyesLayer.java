...
@OnlyIn(Dist.CLIENT)
public class WindsweptHillsEndermanEyesLayer<E extends WindsweptHillsEndermanEntity> extends GeoLayerRenderer<E> {

    public WindsweptHillsEndermanEyesLayer(IGeoRenderer<E> entityRendererIn) {
    ...
    public void render(PoseStack stack, MultiBufferSource buffer, int packedLightIn, E creeper, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
        GeoModel normalModel = this.getEntityModel().getModel(WindsweptHillsEndermanModel.MODEL);
        VertexConsumer glowConsumer = buffer.getBuffer(RenderTypes.getTransparentEyes(WindsweptHillsEndermanModel.GLOW_TEXTURE));
