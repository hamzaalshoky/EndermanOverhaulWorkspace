...
@OnlyIn(Dist.CLIENT)
public class NetherWastesEndermanEyesLayer<E extends NetherWastesEndermanEntity> extends GeoLayerRenderer<E> {

    public NetherWastesEndermanEyesLayer(IGeoRenderer<E> entityRendererIn) {
    ...
    public void render(PoseStack stack, MultiBufferSource buffer, int packedLightIn, E creeper, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
        GeoModel normalModel = this.getEntityModel().getModel(NetherWastesEndermanModel.MODEL);
        VertexConsumer glowConsumer = buffer.getBuffer(RenderTypes.getTransparentEyes(NetherWastesEndermanModel.GLOW_TEXTURE));
