...
@OnlyIn(Dist.CLIENT)
public class WarpedForestEndermanEyesLayer<E extends WarpedForestEndermanEntity> extends GeoLayerRenderer<E> {

    public WarpedForestEndermanEyesLayer(IGeoRenderer<E> entityRendererIn) {
    ...
    public void render(PoseStack stack, MultiBufferSource buffer, int packedLightIn, E creeper, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
        GeoModel normalModel = this.getEntityModel().getModel(WarpedForestEndermanModel.MODEL);
        VertexConsumer glowConsumer = buffer.getBuffer(RenderTypes.getTransparentEyes(WarpedForestEndermanModel.GLOW_TEXTURE));
