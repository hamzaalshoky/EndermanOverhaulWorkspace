...
@OnlyIn(Dist.CLIENT)
public class CrimsonEndermanEyesLayer<E extends CrimsonForestEndermanEntity> extends GeoLayerRenderer<E> {

    public CrimsonEndermanEyesLayer(IGeoRenderer<E> entityRendererIn) {
    ...
    public void render(PoseStack stack, MultiBufferSource buffer, int packedLightIn, E creeper, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
        GeoModel normalModel = this.getEntityModel().getModel(CrimsonForestEndermanModel.MODEL);
        VertexConsumer glowConsumer = buffer.getBuffer(RenderTypes.getTransparentEyes(CrimsonForestEndermanModel.GLOW_TEXTURE));
